...
class AnonymizerConfigurator():
    ...
    def __init__(self, _field_pattern_mapping, 
                 ...
                 _no_of_values_to_generate=10000,
                 _faker_seed=0) -> pd.Series:
    ...
    def __build_replacement_value_lists(self):
        ...
        if self.custom_replacement_values is not None:
            for regex_group_key in self.custom_replacement_values:
                self.replacement_values[regex_group_key] = self.custom_replacement_values[regex_group_key]
    ...
    def __init__(self, row, replacement_dict, field_pattern_dict) -> None:
        ...
        self.function_map = {
            ...
            'fname': {
                ...
                'replacement_list': self.replacement_dict['fname'],
                'current_value': None
            },
            'lname': {
                'function': self.__get_anonymized_value,
                'replacement_list': self.replacement_dict['lname'],
                'current_value': None
            },
            'email': {
                'function': self.__get_anonymized_value,
                'replacement_list': self.replacement_dict['email'],
                'current_value': None
            },
            'street_name': {
                'function': self.__get_anonymized_value,
                'replacement_list': self.replacement_dict['street_name'],
                'current_value': None
            },
            'city': {
                'function': self.__get_anonymized_value,
                'replacement_list': self.replacement_dict['city'],
                'current_value': None
            },
        ...
        if self.replacement_dict is not None:
            for replacement_key in self.replacement_dict:
                self.function_map[replacement_key] = {
                    'function': self.__get_anonymized_value,
                    'replacement_list': self.replacement_dict[replacement_key],
                    'current_value': None
                }
        ...
        for field, pattern in self.field_pattern_dict.items():
            ...
            if match:
                ...
                for match_group_name, match_group_value in match_groups.items():
                    ...
                    if type(match_group_randomizer) is dict:
